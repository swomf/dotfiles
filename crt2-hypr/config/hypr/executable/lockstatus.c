...
#define RECT_LEN 16
#define DATE_LEN 11
#define MEM_LEN 4 // e.g. 123.5 is too long but 841 isnt
...
int main() {
  ...
  snprintf(mem_used_str, MEM_LEN, "%3.1f%s", mem.used,
           data_size_str[mem.used_unit]);
  snprintf(mem_total_str, MEM_LEN, "%3.1f%s", mem.total,
...
int update_date(char *date_str, size_t date_len) {
  ...
  if (strftime(date_str, date_len, "%Y-%m-%d", tm) == 0) {
    return -1;
  }
  return 0;
}
...
int update_memory(Mem *mem) {
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {
    if (strncmp(line, "MemTotal:", 9) == 0) {
      ...
      sscanf(line, "Cached: %f kB", &mem->used);
    }
  }
  fclose(fp);
